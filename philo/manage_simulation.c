...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/01/18 14:48:25 by elara-va          #+#    #+#             */
/*   Updated: 2026/01/18 14:56:38 by elara-va         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

static void	spawn_philos_and_monitor(t_resources *resources,
	int *created_philos, int *return_value, pthread_t *monitor)
{
	while (*created_philos < resources->requested_philos && *return_value == 1)
	{
		if (pthread_create(&resources->curr_philo_node->thread, NULL,
				philosophers_routine, resources) != 0)
		{
		...
		{
			wait_for_node_to_be_ready(resources);
			(*created_philos)++;
		}
	}
	if (*return_value == 1
		&& pthread_create(monitor, NULL, monitor_routine, resources) != 0)
		*return_value = 0;
	return ;
}

static int	kick_off_simulation(t_resources *resources, int created_philos,
	int return_value, pthread_t monitor)
{
...
static int	odds_eat(t_resources *resources, int created_philos,
	int return_value, pthread_t monitor)
{
	...
	while (!pthread_mutex_lock(&resources->odds_meal_count_lock)
		&& resources->odds_meal_count != resources->odd_philos)
	{
	...
	resources->odds_eat_flag = 0; // This doesn't need a lock
	// because when we exit the previous while loop
	// the threads will be doing the other tasks or waiting
	// for the meal count to restart (below)
	resources->odds_meal_count = 0;
	pthread_mutex_unlock(&resources->odds_meal_count_lock);
	return (1);
}

static int	evens_eat(t_resources *resources, int created_philos,
	int return_value, pthread_t monitor)
{
	...
	while (!pthread_mutex_lock(&resources->evens_meal_count_lock)
		&& resources->evens_meal_count != resources->even_philos)
	{
	...
	resources->evens_eat_flag = 0; // This doesn't need a lock
	// because when we exit the previous while loop
	// the threads will be doing the other tasks or waiting
	// for the meal count to restart (below)
...
int	manage_simulation(t_resources *resources)
{
	...
	spawn_philos_and_monitor(resources, &created_philos,
